import { inspectItemAdvanced, parseMonkey } from './utils'
import { range } from '../utils/numbers-utils'
import { Monkey } from './types'
import { logResponse } from '../utils/log-utils'

const TASK_DATA = ['11', 'Advanced']
const TASK_LABEL = TASK_DATA.join(' ')

export const advanced_11 = async (inputContent: string[], monkeysCount: number, roundsLimit: number, logIt: boolean) => {
  ...
  const lcm = monkeys.map(monkey => monkey.test).reduce((acc, val) => acc * val, 1);

  rounds.forEach(round => {
    monkeys.forEach((monkey, currentMonkey) => {
      while (monkey.items.length > 0) {
        const [item, monkeyIndex] = inspectItemAdvanced(monkey, lcm);
